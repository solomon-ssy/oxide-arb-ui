...
import {
  ...
  useSettlementSubmissionKindTagOptions,
...
const stateTagOptions = useSettlementCaseStateTagOptions();
const effectivePolicyTagOptions = useSettlementEffectivePolicyTagOptions();
const submissionKindTagOptions = useSettlementSubmissionKindTagOptions();
...
const effectivePolicyTag = computed(() =>
  findTagOption(effectivePolicyTagOptions, header.value?.effective_policy),
);
...
const headerTags = computed(() => {
  ...
  const state = findTagOption(stateTagOptions, current.state);
  return [
    { color: state?.color, label: state?.label ?? current.state },
    {
      color: effectivePolicyTag.value?.color,
      label: effectivePolicyTag.value?.label ?? current.effective_policy,
    },
  ];
});
...
function submissionKindTag(kind: string) {
  return findTagOption(submissionKindTagOptions, kind);
}
