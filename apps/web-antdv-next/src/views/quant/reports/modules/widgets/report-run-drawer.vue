...
async function retry() {
  ...
  if (result) {
    await router.replace({
      path: '/quant/reports',
      query: { ...route.query, run_id: result.report_run_id },
    });
...
watch(
  () => route.query.run_id,
  (id) => {
    if (typeof id !== 'string' || !id || id === runId.value) return;
    runId.value = id;
    void load();
  },
);
      ...
      <Descriptions :column="2" bordered size="small">
        ...
        <DescriptionsItem :label="$t('page.quantReports.runs.retryOf')">
          <EntityRouteLink
            ...
            :to="`/quant/reports?run_id=${run.retry_of_run_id}`"
          />
          <span v-else>—</span>
        </DescriptionsItem>
        <DescriptionsItem :label="$t('page.quantReports.runs.outputReport')">
          <EntityRouteLink
            ...
            :to="`/quant/reports/${run.output_report_id}`"
          />
          <span v-else>—</span>
        </DescriptionsItem>
      </Descriptions>
