...
const bookAgeMs = computed<null | number>(() => {
  const side = book.value?.yes ?? book.value?.no;
  return side ? Date.now() - side.timestamp_ms : null;
});
...
function windowForRange(): { from: string; to: string } {
  const span = RANGE_OPTIONS.find((option) => option.value === range.value);
  const toMs = Date.now();
  const fromMs = toMs - (span?.ms ?? DEFAULT_RANGE_MS);
  return {
