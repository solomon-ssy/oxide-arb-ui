...
/** Remove null-only leading/trailing time buckets while preserving real gaps. */
export function trimEmptyEdgePoints(points: BucketPoint[]): BucketPoint[] {
  let start = 0;
  while (start < points.length && points[start]?.[1] === null) {
    start += 1;
  }

  let end = points.length;
  while (end > start && points[end - 1]?.[1] === null) {
    end -= 1;
  }
...
/** Show points when a sparse series has no time-progressing line segment. */
export function shouldShowPointSymbols(points: BucketPoint[]): boolean {
...
/** Fixed tick spacing for each selectable chart horizon. */
export function timeTickInterval(windowMs: number): number {
  if (windowMs <= 5 * 60_000) return 60_000;
  if (windowMs <= 60 * 60_000) return 10 * 60_000;
  if (windowMs <= 6 * 60 * 60_000) return 60 * 60_000;
  if (windowMs <= 24 * 60 * 60_000) return 4 * 60 * 60_000;
  return 12 * 60 * 60_000;
}

/** Format an x-axis timestamp as its age relative to the live window head. */
export function relativeTimeLabel(timestamp: number, anchor: number): string {
  const elapsedSeconds = Math.max(0, Math.round((anchor - timestamp) / 1000));
