...
import FeedbackDriftPanel from './modules/feedback-drift-panel.vue';
...
type WorkbenchView = 'cycles' | 'drift' | 'overview';
...
const emptyDrifts: Paginated<DriftReportView> = {
  has_next: false,
  items: [],
  page: 1,
  size: PAGE_SIZE,
...
const driftPage = ref<Paginated<DriftReportView>>(emptyDrifts);
const currentPage = ref(1);
const driftCurrentPage = ref(1);
...
const driftError = ref<null | string>(null);
...
const driftReadCoordinator = new AuthoritativeReadCoordinator<
  number,
  Paginated<DriftReportView>
>({
  async fetchSnapshot(page, signal) {
    return listDriftReports({ page, size: PAGE_SIZE }, { signal });
  },
  initialKey: driftCurrentPage.value,
  onError() {
    driftError.value = $t('page.research.feedback.drift.loadError');
  },
  onPendingChange(pending) {
    driftLoading.value = pending;
  },
  onSnapshot(snapshot) {
    driftError.value = null;
    driftPage.value = snapshot;
  },
});
...
async function refreshDrifts() {
  if (!canRead.value) {
    driftReadCoordinator.cancel();
    driftPage.value = emptyDrifts;
    driftLoading.value = false;
    driftError.value = null;
    return;
  }
  driftReadCoordinator.setKey(driftCurrentPage.value);
  await driftReadCoordinator.refresh();
}
...
watch(driftCurrentPage, (page) => {
  if (canRead.value) {
    driftReadCoordinator.changeKey(page);
  }
});
watch(
  routeCycleId,
...
watch(
  ...
  () => {
    if (!leaving && canRead.value) {
      ...
      driftReadCoordinator.invalidate();
    ...
    <div v-else class="space-y-4">
      ...
      <template v-else>
        ...
        >
          ...
          <TabPane
            key="drift"
            :tab="$t('page.research.feedback.tabs.drift')"
            :force-render="true"
          >
            <FeedbackDriftPanel
              :current-page="driftCurrentPage"
              :error="driftError"
              :loading="driftLoading"
              :page="driftPage"
              @page="driftCurrentPage = $event"
              @retry="refreshDrifts"
            />
          </TabPane>
        </Tabs>
