...
function openActiveJob() {
  ...
  void router.push(
    `/runtime/activity?domain=research&entity=research-job&id=${props.activeJobId}`,
  );
}
    ...
    <Descriptions bordered :column="2" size="small">
      ...
      <DescriptionsItem :label="$t('page.research.cpcv.dataset')">
        <EntityRouteLink
          ...
          :to="`/research/lab?module=datasets&entity=training-dataset&id=${pathSet.training_dataset_id}`"
        />
      </DescriptionsItem>
      <DescriptionsItem
