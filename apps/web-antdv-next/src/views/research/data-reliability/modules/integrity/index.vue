...
async function runFull() {
  ...
  if (job) {
    ...
    await router.push(
      `/runtime/activity?domain=research&entity=research-job&id=${job.job_id}`,
    );
  }
}
...
async function acknowledgeLatch() {
  ...
  if (run.model_version_id) {
    details.push({
      ...
      mono: true,
      routeTo: `/research/lab?module=models&entity=model-version&id=${run.model_version_id}`,
  ...
  if (run.training_dataset_id) {
    details.push({
      ...
      mono: true,
      routeTo: `/research/lab?module=datasets&entity=training-dataset&id=${run.training_dataset_id}`,
...
function applyRunDeepLink() {
  const runId = featureIntegrityRunIdFromQuery(
    route.query.entity,
    route.query.id,
  );
  if (!runId) {
...
watch(() => [route.query.entity, route.query.id], applyRunDeepLink);
      ...
      <Card v-if="latch?.open" size="small">
        <Descriptions :column="2" bordered size="small">
          ...
          >
            ...
            <RouterLink
              v-else
              class="blocking-run-link font-mono text-xs break-all"
        ...
        >
          <Descriptions :column="2" bordered size="small">
            <DescriptionsItem
              ...
              :key="state"
              :label="
                enumOption(featureCellStateOptions, state)?.label ??
                EMPTY_PLACEHOLDER
              "
            >
              {{ count }}
